...
function Body({spotifyApi, chooseTrack}) {
  ...
  return (
    <section className='bg-neutral-900 ml-24
    py-4 space-y-8 md:max-w-6x1 flex-grow md:mr-2.5'>
      <Search search={search} setSearch={setSearch}/>
      <div className='grid overflow-y-scroll scrollbar-hide
      h-96 py-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4
      gap-x-4 gap-y-8 p-4'>
      ...
      <div className='flex gap-x-8 absolute min-w-full md:relative ml-6'>
        <div className="hidden xl:inline max-w-[270px]">
            ...
            <button className="text-[#CECECE] bg-[#1A1A1A] text-[13px] 
            py-3.5 px-4 rounded-2xl w-full font-bold bg-opacity-80 
